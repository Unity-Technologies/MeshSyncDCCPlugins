...
def select_link_function(self, tag):
    ...
    def get_package_name():
        ...
        os = platform.system()
        if os == 'Linux':
            return 'UnityMeshSync_Blender_Linux.zip'
        elif os == 'Darwin':
            return 'UnityMeshSync_Blender_Mac.zip'
        elif os == 'Windows':
            return 'UnityMeshSync_Blender_Windows.zip'
        return None

    from urllib.parse import urljoin
    url = urljoin("https://github.com/Unity-Technologies/MeshSyncDCCPlugins/blob/",tag["name"]+"/")
    url = urljoin(url, "Editor/Plugins/")
    url = urljoin(url, get_package_name())
    url = url+ "?raw=true"
    return url
...
def register(bl_info):
    ...
    updater.user = "Unity-Technologies"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "MeshSyncDCCPlugins"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/Unity-Technologies/MeshSyncDCCPlugins"
    ...
    version = msb_context.PLUGIN_VERSION
